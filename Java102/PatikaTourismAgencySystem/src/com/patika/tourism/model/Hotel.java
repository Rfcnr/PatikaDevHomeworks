...
public class Hotel {
    ...
    public static boolean add(byte star, String name, String country, String region, String city, String district, String address, String eMail, String phone, boolean freeParking, boolean freeWiFi, boolean swimmingPool, boolean fitness, boolean concierge, boolean spa, boolean allTimeRoomService, ArrayList<Season> seasons){
        ...
        if(fetchHotelFromDbByName(name) != null){
            MyHelper.showMessage("Hotel : \"" + name + "\" is already exist.", "Hotel Name Error!");
            return false;
        }

        try {
            Connection connection = MyDbConnector.getInstance();
            connection.setAutoCommit(false);

            // Add Hotel
            String queryHotel = "INSERT INTO "+ MyDbConstants.DB_TN_HOTEL + " (star, name, country, region, city, district, address, email, phone, free_parking, free_wifi, swimming_pool, fitness, concierge, spa, all_time_room_service) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
            ...
            if(!resultAddingHotel){
                MyHelper.showMessage("There was an error, the hotel could not be added!", "Hotel Adding Error!");
                return false;
            }
            ...
            for (Season season : seasons){
                ...
                String querySeason = "INSERT INTO " + MyDbConstants.DB_TN_SEASON + " (start_date, end_date) VALUES (?, ?)";
            ...
            if(resultAddingSeason){
                for (Season season : seasons){
                    ...
                    if(temp != null){
                        ...
                        String queryItemHotelSeason = "INSERT INTO " + MyDbConstants.DB_TN_HOTEL_SEASON + " (hotel_id, season_id) VALUES (?, ?)";
            ...
            if(!resultAddingSeason || !resultAddingHotelSeason){
                MyHelper.showMessage("There was an error, the hotel season could not be added!", "Hotel Season Adding Error!");
                connection.rollback();
                return false;
            }
            else result = true;
    ...
    public static Hotel fetchHotelFromDbByName(String name){
        Hotel hotel = null;
        String query = "SELECT * FROM " + MyDbConstants.DB_TN_HOTEL + " WHERE name = ?";
    ...
    public static ArrayList<Hotel> getList(){
        ...
        String query = "SELECT * FROM " + MyDbConstants.DB_TN_HOTEL;
        Hotel hotel;

        try {
            Statement st = MyDbConnector.getInstance().createStatement();
    ...
    public static Hotel fetchHotelByID(int id){
        ...
        String query = "SELECT * FROM " + MyDbConstants.DB_TN_HOTEL + " WHERE id = ?";
