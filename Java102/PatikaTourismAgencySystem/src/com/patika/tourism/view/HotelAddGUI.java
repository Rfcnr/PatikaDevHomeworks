...
import com.patika.tourism.model.Hotel;
import com.patika.tourism.model.Season;
...
public class HotelAddGUI extends JFrame{
    ...
    public HotelAddGUI(JFrame previousFrame) {
        ...
        addSeasonsButton.addActionListener(e -> {
            if(MyHelper.isFieldEmpty(fld_season_start_date, fld_season_end_date)){
                MyHelper.showMessage("You have not entered the start and end dates.", "Missing Date Info");
            }
            else {
                ...
                if(startDate.isAfter(endDate) || startDate.isEqual(endDate)){
                    MyHelper.showMessage("The start date cannot be the same or later than the end date!", "Invalid Entry!");
                    return;
                } else if (endDate.isEqual(LocalDate.now()) || endDate.isBefore(LocalDate.now())) {
                    MyHelper.showMessage("Past seasons cannot be recorded!", "Invalid Entry!");
                }
                ...
                for (int i = 0; i < startDates.length; i++) {
                    if(!startDates[i].equals("-")){
                        ...
                        if(startDate.isEqual(labelStartDate) || startDate.isEqual(labelEndDate) || endDate.isEqual(labelStartDate) || endDate.isEqual(labelEndDate)){
                            MyHelper.showMessage("Season start or end dates cannot be the same!", "Invalid Entry!");
                            datesAreAvailable = false; break;
                        }
                        ...
                        {
                            MyHelper.showMessage("Seasons cannot contain the same days!", "Invalid Entry!");
                            datesAreAvailable = false; break;
                        }
                    }
                }
                ...
                if(datesAreAvailable){
                    if(lbl_s1_start.getText().equals("-")){
                        ...
                        MyHelper.showMessage("No more than 3 seasons can be recorded!", "Season registrations are full!");
                    }
                }
            }
            sortSeasons();
        ...
        btn_clear_season1_label.addActionListener(e -> {
            if(!lbl_s1_start.getText().equals("-")){
                ...
                if (MyHelper.confirm(message, "Attention!")){
        ...
        btn_new_hotel_cancel.addActionListener(e -> {
            if(MyHelper.confirm("Are you sure you want to cancel the transaction?", "Cancel Transaction!")){
                previousFrame.setEnabled(true);
                dispose();
            }
        });
        ...
        btn_new_hotel_fields_clear.addActionListener(e -> {
            if(MyHelper.confirm("All the data you entered will be deleted!\nDo you agree?", "Attention!")){
        ...
        btn_new_hotel_add.addActionListener(e -> {
            ...
            if(MyHelper.isFieldEmpty(fld_name, fld_country,fld_region, fld_city, fld_district, fld_address, fld_e_mail, fld_phone)){
                MyHelper.showMessage("Please fill in all fields.", "Incomplete Data Entry!");
                return;
            }

            String name = fld_name.getText();
            if (name.length() > 100){
                MyHelper.showMessage("The hotel name can be up to 100 characters.", "Name Size Error!");
                return;
            }

            String country = fld_country.getText();
            if (country.length() > 100){
                MyHelper.showMessage("The country name can be up to 100 characters.", "Name Size Error!");
                return;
            }

            String region = fld_region.getText();
            if (region.length() > 100){
                MyHelper.showMessage("The region name can be up to 100 characters.", "Name Size Error!");
                return;
            }

            String city = fld_city.getText();
            if (city.length() > 100){
                MyHelper.showMessage("The city name can be up to 100 characters.", "Name Size Error!");
                return;
            }

            String district = fld_district.getText();
            if (district.length() > 100){
                MyHelper.showMessage("The district name can be up to 100 characters.", "Name Size Error!");
                return;
            }

            String address = fld_address.getText();
            if (address.length() > 255){
                MyHelper.showMessage("The address can be up to 100 characters.", "Name Size Error!");
                return;
            }

            String eMail = fld_e_mail.getText();
            if (eMail.length() > 100){
                MyHelper.showMessage("E-mail can be up to 100 characters.", "Name Size Error!");
                return;
            }

            String phone = fld_phone.getText();
            if (phone.length() > 20){
                MyHelper.showMessage("Phone number can be up to 20 characters.", "Name Size Error!");
                return;
            }

            byte star;
            if(cmb_stars.getSelectedIndex() == 0){
                MyHelper.showMessage("Select how many stars the hotel has.", "Hotel Star Error!");
                return;
            }
            ...
            DateTimeFormatter dtFormatter = DateTimeFormatter.ofPattern("dd.MM.yyyy");
            if(!lbl_s1_start.getText().equals("-")) seasons.add(new Season(LocalDate.parse(lbl_s1_start.getText(), dtFormatter), LocalDate.parse(lbl_s1_end.getText(), dtFormatter)));
            if(!lbl_s2_start.getText().equals("-")) seasons.add(new Season(LocalDate.parse(lbl_s2_start.getText(), dtFormatter), LocalDate.parse(lbl_s2_end.getText(), dtFormatter)));
            if(!lbl_s3_start.getText().equals("-")) seasons.add(new Season(LocalDate.parse(lbl_s3_start.getText(), dtFormatter), LocalDate.parse(lbl_s3_end.getText(), dtFormatter)));

            if(seasons.size() == 0){
                MyHelper.showMessage("At least one season must be added!", "No Season Error!");
            }
            else {
                // New Hotel Adding
                ...
                {
                    MyHelper.showMessage("A new hotel has been added.", "Success");
                    previousFrame.setEnabled(true);
                    dispose();
                }
            }
